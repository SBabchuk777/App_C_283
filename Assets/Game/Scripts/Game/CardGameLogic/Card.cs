...
namespace Game.Scripts.Game.CardGameLogic
{
    public class Card : MonoBehaviour
    {
        ...
        [ContextMenu("MakeOpenAnimation")]
        public void PlayCloseAnimation()
        {
            OpenImage.gameObject.SetActive(true);
            CloseImage.gameObject.SetActive(false);

            OpenImage.transform.DORotate(new Vector3(90, 90, 0), RotateDuration / 2).OnComplete((() =>
            {
                CloseImage.gameObject.SetActive(true);
                OpenImage.gameObject.SetActive(false);

                OpenImage.transform.rotation = Quaternion.Euler(Vector3.zero);
                CloseImage.transform.rotation = Quaternion.Euler(new Vector3(90, 90, 0));
                _animationTween = CloseImage.transform.DORotate(new Vector3(0, 0, 0), RotateDuration / 2);
            }));
        }

        private void UpdateOpenImage() =>
            OpenImage.sprite = CardData.OpenIcon;
    }
}